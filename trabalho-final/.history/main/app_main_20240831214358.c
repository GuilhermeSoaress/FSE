...
void dht_test(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        //mqtt_envia_mensagem("/topic/luminosidade", luminosidade);
        ...
        if (dht_read_float_data(DHT_TYPE_DHT11, 5, &humidity, &temperature) == ESP_OK){
            ...
            // mqtt_envia_mensagem("v1/devices/me/attributes/temperatura ", temperatura);
            // mqtt_envia_mensagem("v1/devices/me/attributes/umidade", umidade);
...
int led_state = 0; // Estado inicial do LED
// Função de interrupção do GPIO
...
void led_control_task(void *arg) {
    ...
    for (;;) {
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            ...
            gpio_set_level(LED_GPIO, led_state); // Atualiza o LED
...
void app_main(void)
{
    ...
    gpio_config_t io_conf = {
        .intr_type = GPIO_INTR_NEGEDGE, // Interrupção na borda de descida (botão pressionado)
        .mode = GPIO_MODE_INPUT, // Modo de entrada para o botão
        .pin_bit_mask = (1ULL << BUTTON_GPIO), // GPIO do botão
    ...
    gpio_set_level(LED_GPIO, led_state); // Define o estado inicial do LED como desligado

    // Cria uma fila para eventos de GPIO
    gpio_evt_queue = xQueueCreate(10, sizeof(uint32_t));
    // Inicia a task de controle do LED
    xTaskCreate(led_control_task, "led_control_task", 2048, NULL, 10, NULL);

    // Instala o serviço de interrupção de GPIO e adiciona o handler
