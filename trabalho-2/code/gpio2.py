...
class GPIOController:
    def __init__(self):
        ...
        self.DIR1_PIN = 19  # GPIO 20 (BCM) ou Pino 38 (Board)
        self.DIR2_PIN = 26  # GPIO 21 (BCM) ou Pino 40 (Board)
        self.PWM_PIN = 13   # GPIO 12 (BCM) ou Pino 32 (Board)
        self.SENSOR_TERREO_PIN = 17  # GPIO 18 (BCM) ou Pino 12 (Board)
        self.SENSOR_1_ANDAR_PIN = 27 # GPIO 23 (BCM) ou Pino 16 (Board)
        self.SENSOR_2_ANDAR_PIN = 22 # GPIO 24 (BCM) ou Pino 18 (Board)
        self.SENSOR_3_ANDAR_PIN = 6 # GPIO 25 (BCM) ou Pino 22 (Board)
    ...
    def ir_para_andar(self, andar):
        ...
        self.pid = PID()
        self.start_pwm()
        self.andar_atual = cmds2.apurar_encoder()
        ...
        if andar == 0:
            ...
            if self.andar_atual - self.terreo > 40:
                while self.andar_atual > self.terreo + 40:
                    print(self.pid.controle(cmds2.apurar_encoder()))
                    self.aciona_motor('Sobe', self.pid.controle(cmds2.apurar_encoder()))
            ...
            if self.andar_atual - self.andar1 > 40:
                while self.andar_atual > self.andar1 + 40:
                    ...
                    print(self.pid.controle(cmds2.apurar_encoder()))
                    self.aciona_motor('desce', abs(self.pid.controle(cmds2.apurar_encoder())))
                    sleep(0.1)
                    pwm_global = self.pid.controle(self.andar_atual)
                    cmds2.apurar_pwm()
                self.stop_pwm()
                ...
                while self.andar_atual < self.andar1 - 40:
                    self.aciona_motor('sobe', abs(self.pid.controle(cmds2.apurar_encoder())))
                    logging.debug(f"Andar atual: {self.andar_atual}")
                    sleep(0.1)
                    pwm_global = self.pid.controle(self.andar_atual)
                    cmds2.apurar_pwm()
