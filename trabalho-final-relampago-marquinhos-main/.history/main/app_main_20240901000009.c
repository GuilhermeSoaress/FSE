...
void process_json(const char *json_str) {
    ...
    // Acesse o objeto 'params'
    cJSON *params = cJSON_GetObjectItemCaseSensitive(json, "params");
    if (params == NULL || !cJSON_IsObject(params)) {
        fprintf(stderr, "Não foi possível encontrar o objeto 'params'\n");
        cJSON_Delete(json);
        return;
    }

    // Iterar sobre todos os itens em 'params'
    cJSON *item = NULL;
    cJSON_ArrayForEach(item, params) {
        // Obtenha o nome do parâmetro
        const char *param_name = item->string;
        // Obtenha o valor do parâmetro
        if (cJSON_IsBool(item)) {
            bool value = cJSON_IsTrue(item);
            printf("Parâmetro: %s, Valor: %s\n", param_name, value ? "true" : "false");
        } else if (cJSON_IsNumber(item)) {
            printf("Parâmetro: %s, Valor: %d\n", param_name, item->valueint);
        } else if (cJSON_IsString(item)) {
            printf("Parâmetro: %s, Valor: %s\n", param_name, item->valuestring);
        } else {
            printf("Parâmetro: %s, Tipo de valor não suportado\n", param_name);
        }
    }

    // Libere a memória alocada para o JSON
    cJSON_Delete(json);
}
