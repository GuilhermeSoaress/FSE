...
int i =1;
void configuraPinos(){
    esp_rom_gpio_pad_select_gpio(botaoFarois);
    gpio_set_direction(botaoFarois, GPIO_MODE_INPUT);
    gpio_pulldown_en(botaoFarois);
    gpio_pullup_dis(botaoFarois);
    ...
    gpio_set_intr_type(botaoFarois, GPIO_INTR_ANYEDGE);
    gpio_set_intr_type(botaoVidros, GPIO_INTR_ANYEDGE);
    gpio_set_intr_type(botaoTrava, GPIO_INTR_ANYEDGE);
}
    ...
    if(xQueueReceive(filaDeInterrupcao, &pino, portMAX_DELAY))
    {
      ...
      int estado = gpio_get_level(pino);
        ...
        case botaoFarol:
            ...
            farolStatus = estado; 
            snprintf(attributes, sizeof(attributes), "{\"farol\": %d, \"botaoVidros\": %d, \"botaoTravar\": %d, \"travadoStatus\": %d}", farolStatus,botaoVidrosStatus, botaoTravarStatus);
            mqtt_envia_mensagem("v1/devices/me/attributes", attributes);
            
            sprintf(farol, "%d", farolStatus);
            mqtt_envia_mensagem2("sensores/farol/" , farolStatus);
            break;
        
        case botaoVidros:
            
            snprintf(attributes, sizeof(attributes), "{\"farol\": %d, \"botaoVidros\": %d, \"botaoTravar\": %d, \"travadoStatus\": %d}", farolStatus,1, botaoTravarStatus);
            mqtt_envia_mensagem("v1/devices/me/attributes", attributes);
        ...
        case botaoTrava:
            ...
            snprintf(attributes, sizeof(attributes), "{\"farol\": %d, \"botaoVidros\": %d, \"botaoTravar\": %d, \"travadoStatus\": %d}", farolStatus, botaoVidrosStatus,1);
            mqtt_envia_mensagem("v1/devices/me/attributes", attributes);

            sprintf(botaoVidrosStr, "%d", 1);
            mqtt_envia_mensagem2("sensores/trava", botaoVidrosStr);
            break;
...
{   
    ...
    while (1)
    {
        ...
        //sprintf(farol,"%d", farolStatus);
        //sprintf(luzInterna, "%d", luzInternaPwm);
        ...
        //mqtt_envia_mensagem2("sensores/farol", farolStatus);
        //mqtt_envia_mensagem2("sensores/luzInterna", luzInternaPwm);
