...
void separa_params(const char *json_str, method, params) {
    // Parse o JSON
    ...
    // Obtém os valores das chaves "method" e "params"
    struct json_object *method_obj = json_object_object_get(obj, "method");
    struct json_object *params_obj = json_object_object_get(obj, "params");

    // Copia os valores para as strings de saída
    strcpy(method, json_object_get_string(method_obj));
    strcpy(params, json_object_get_string(params_obj));

    // Libera a memória alocada pelo json-c
    json_object_put(obj);
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            bool valor;
            valor = params;
           if(strcmp(funcao, "farol")==0){
                farolStatus = farois(valor);
                printf("Status do farol %b
