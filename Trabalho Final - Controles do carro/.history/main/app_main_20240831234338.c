...
// Variável global para armazenar o estado do farol
int farol_status = 0;

// Função para processar a mensagem MQTT
void process_mqtt_message(const char* payload) {
    ...
    // Obtém o objeto "params"
    cJSON *params = cJSON_GetObjectItemCaseSensitive(json, "params");
    if (params == NULL) {
        printf("Não encontrou o objeto 'params'\n");
        cJSON_Delete(json);
        return;
    }

    // Obtém o valor de "farol"
    cJSON *farol_item = cJSON_GetObjectItemCaseSensitive(params, "farol");
    if (cJSON_IsBool(farol_item)) {
        // Acessa o valor booleano e armazena na variável
        farol_status = cJSON_IsTrue(farol_item);
        printf("Valor de 'farol' armazenado: %d\n", farol_status);
    } else {
        printf("O valor de 'farol' não é um booleano.\n");
    }
...
void dht_test(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        process_mqtt_message(example_payload);

    // Agora você pode usar a variável farol_status em outras partes do código
    if (farol_status) {
        printf("O farol está ligado.\n");
    } else {
        printf("O farol está desligado.\n");
    }

        sprintf(attributes, "{\"temperatura\": %.2f, \"umidade\": %.2f, \"luminosidade\" : %s}", temperature, humidity, luminosidade);
        mqtt_envia_mensagem("v1/devices/me/telemetry", attributes);
